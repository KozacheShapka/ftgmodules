            ...
            if message.mentioned:
                if repstatus is not False:
                    if message.text == "+":
                        ...
                        await message.reply(f"<b>Ей уебок, ты повысил мою репутацию!\nТеперь моя конченнная репутация: {number}.</b>")
                    elif message.text == "-":
                        total = int(number) - 1
                        self.db.set("MyRep", "my_repa", total)
                        await message.reply(f"<b>Ей уебок, ты понизил сука мою репутацию!\nТеперь моя конченнная репутация: {total}.</b>")
        except: pass